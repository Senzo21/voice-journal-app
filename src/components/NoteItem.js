...
/**
 * Props:
 *  - note {id,title,uri,createdAt,duration}
 *  - playing (bool)
 *  - progress (0..1)
 *  - onPlay(opts) -> opts: {reverse:boolean}
 *  - onPause()
 *  - onDelete()
 *  - onRename(newTitle)
        ...
        <View style={styles.card}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.meta}>
              {new Date(note.createdAt).toLocaleString()} · {Math.round((note.duration || 0) / 1000)}s
            </Text>

            {/* progress bar */}
            <View style={styles.progressContainer}>
              <Animated.View style={[styles.progressFill, { width: progAnim.interpolate({ inputRange: [0, 1], outputRange: ["0%", "100%"] }) }]} />
            </View>
      ...
      {speedMenuVisible && (
        <View style={styles.speedMenu}>
          {[1.0, 1.5, 2.0].map((r) => (
            ...
            >
              <Text style={{ color: r === currentRate ? "#fff" : "#bfbfbf", fontWeight: r === currentRate ? "800" : "500" }}>{r}x</Text>
            </TouchableOpacity>
          ))}
        </View>
      )}
      ...
      <Modal visible={renameVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <TextInput value={newTitle} onChangeText={setNewTitle} style={styles.renameInput} placeholder="Enter title" placeholderTextColor="#777" />
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 16 }}>
              ...
              <TouchableOpacity
                onPress={() => {
                  onRename(newTitle.trim() || note.title);
...
const styles = StyleSheet.create({
  ...
  progressContainer: { height: 4, backgroundColor: "#17181A", borderRadius: 4, overflow: "hidden", marginTop: 10, marginBottom: 8 },
  ...
  modalBox: { width: "86%", backgroundColor: "#0f1113", padding: 16, borderRadius: 12, borderWidth: 1, borderColor: "#222" },
  modalTitle: { color: "#fff", fontSize: 17, fontWeight: "700" },
  renameInput: { marginTop: 12, padding: 10, borderRadius: 8, backgroundColor: "#16171A", color: "#fff", borderWidth: 1, borderColor: "#222" },

  speedMenu: { position: "absolute", right: 18, top: 60, backgroundColor: "#0f1112", padding: 8, borderRadius: 8, borderWidth: 1, borderColor: "#222" },
